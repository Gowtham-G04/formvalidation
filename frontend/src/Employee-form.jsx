  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editingEmployeeId) {
        const res = await axios.put(`http://localhost:5000/api/employees/${editingEmployeeId}`, form);
        setMessage(res.data.message);
      ...
      setMessage(err.response?.data?.message || 'Error adding/updating employee');
    }
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="button" onClick={() => setShowDetails(!showDetails)}>Show Employee Details</button>
      </form>
